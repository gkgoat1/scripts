...
if ! "$root_dir/sandbox/daemon/client.py" --socket "$socket" ping >/dev/null 2>&1; then "$build/sandboxd" --socket "$socket" --shim "$build/sandbox.dylib" >/dev/null 2>&1 & daemon_pid=$!; trap 'kill "$daemon_pid" 2>/dev/null || true' EXIT; for _ in {1..30}; do "$root_dir/sandbox/daemon/client.py" --socket "$socket" ping >/dev/null 2>&1 && break; sleep .05; done; fi
export SANDBOX_DAEMON_SOCKET="$socket"
if [[ "$(uname -s)" == Darwin && ! -f "$build/sandbox.dylib" ]]; then
  cc -dynamiclib -Wall -Wextra -O2 -o "$build/sandbox.dylib" "$root_dir/sandbox/macos/sandbox.dylib.c"
  codesign --force --sign - --timestamp=none "$build/sandbox.dylib"
fi
case "$(uname -s)" in
